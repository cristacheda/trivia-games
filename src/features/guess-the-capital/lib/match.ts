import { normalizeCapitalText } from '@/features/guess-the-capital/lib/normalize'
...
export function isAcceptableCapitalAnswer(
  input: string,
  subject: CapitalQuestionSource,
) {
  const normalizedInput = normalizeCapitalText(input)
  ...
  const candidates = Array.from(
    new Set(subject.capitalAliases.map((alias) => normalizeCapitalText(alias))),
  )
