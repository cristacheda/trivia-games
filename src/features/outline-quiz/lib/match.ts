import { normalizeOutlineText } from '@/features/outline-quiz/lib/normalize'
...
export function isAcceptableOutlineAnswer(
  input: string,
  subject: OutlineQuestionSource,
) {
  const normalizedInput = normalizeOutlineText(input)
  ...
  const candidates = Array.from(
    new Set(subject.aliases.map((alias) => normalizeOutlineText(alias))),
  )
