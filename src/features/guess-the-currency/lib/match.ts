import { normalizeCurrencyText } from '@/features/guess-the-currency/lib/normalize'
...
export function isAcceptableCurrencyAnswer(
  input: string,
  subject: CurrencyQuestionSource,
) {
  const normalizedInput = normalizeCurrencyText(input)
  ...
  const candidates = Array.from(
    new Set(subject.aliases.map((alias) => normalizeCurrencyText(alias))),
  ).filter(Boolean)
