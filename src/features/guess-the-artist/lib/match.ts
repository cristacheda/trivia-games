import { normalizeArtistText } from '@/features/guess-the-artist/lib/normalize'
...
export function isAcceptableArtistAnswer(input: string, subject: SongQuestionSource) {
  const normalizedInput = normalizeArtistText(input)
  ...
  const candidates = Array.from(
    new Set(subject.aliases.map((alias) => normalizeArtistText(alias))),
  )
